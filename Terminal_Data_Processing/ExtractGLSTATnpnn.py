...
folder_pattern = re.compile(r"glstat_((-?[\d]+)p([\d]+)_(-?[\d]+)p([\d]+)_(-?[\d]+)p([\d]+)")
...
for filename in os.listdir("."):
    if filename.startswith("glstat_"):
        ...
        theta_1 = f"{match.group(1)}.{match.group(2)}"
        theta_2 = f"{match.group(3)}.{match.group(4)}"
        theta_3 = f"{match.group(5)}.{match.group(6)}"
